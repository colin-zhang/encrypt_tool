...
static int EncryptCBC(unsigned char *in, int inl, unsigned char *out, int *outlen,
               const char *key, const unsigned char *iv, int ivLen) {
...
static int DecryptCBC(unsigned char *in, int inl, unsigned char *out, int *outlen,
               const char *key, const unsigned char *iv, int ivLen) {
...
int EncryptStr(const std::string& key, const std::string& iv,
    ...
    int outlen; 
    int ret = EncryptCBC((unsigned char*)str.c_str(), (int)str.size(),
                out, &outlen,
...
int DecryptStr(const std::string& key, const std::string& iv,
    ...
    ret = DecryptCBC((unsigned char*)en, (int)en_len,
                out, &outlen,
                real_key.c_str(), (unsigned char*)real_iv.c_str(), 16);
    if (ret < 0) {
        delete[] en;
